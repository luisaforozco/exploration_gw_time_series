...
def get_coordinates_gmw(bro_GMW_id:str) -> tuple:
    response = requests.get(f"https://int-publiek.broservices.nl/gm/gmw/v1/objects/{bro_GMW_id}?fullHistory=", headers= {"accept": "application/xml"})
    if response.status_code == 400:
        error_data = response.json()
        description = error_data.get('description', 'Unknown error')
        raise ValueError(f"Bad request objects for BRO GMW ID {bro_GMW_id}: {description}")
    xml = response.text
